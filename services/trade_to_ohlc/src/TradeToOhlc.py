...
class TradeToOhlc:
    ...
    """
    def __init__(self, broker_address: str, input_topic_name: str, output_topic_name: str, window_duration_secs: int = 10):
        
        self.app = Application(broker_address=broker_address, consumer_group="trade_to_ohlc_group")
    ...
    def run(self):
        ...
        # Filter out bid/ask updates
        sdf = sdf.filter(lambda row: 'bid' not in row)
        ...
        def reduce_func(aggregated:dict, value:dict) -> dict:
            result = {
                ...
                "close": value["price"],
                "volume": aggregated["volume"] + value["qty"]
            }
        ...
        sdf = (
            sdf.tumbling_window(duration_ms=timedelta(seconds=self.window_duration_secs))
            .reduce(reduce_func, init_func)
            .final()
        )
        ...
        sdf["timestamp_end"] = sdf["start"]
        # For Debugging
        sdf["timestamp_end_str"] = sdf["timestamp_end"].apply(lambda val:datetime.fromtimestamp(val/1000).strftime('%Y-%m-%d %H:%M:%S'))

        sdf = sdf.to_topic(self.output_topic)
        sdf = sdf.apply(logger.info)
...
if __name__ == "__main__":
    ...
    trade_to_ohlc = TradeToOhlc(config.broker_address, config.input_topic_name, config.output_topic_name, config.window_duration_secs)
    trade_to_ohlc.run()